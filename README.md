...
| File | What it holds |
|---|---|
| `data/profile.json` | Name, honorific, headline, tagline, `now` (the status line in the hero), about paragraphs, `work_with_me`, `documents` (CV, research statement, teaching statement: set `file` to a path to show it), `datasets` (`label`, `url`), affiliations, links, skills, languages |
